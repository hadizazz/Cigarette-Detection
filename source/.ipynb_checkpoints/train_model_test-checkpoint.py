...
def fine_tune_model():
    ...
    train_generator = aug.flow(x_train, y_train, batch_size=BS, save_to_dir=PATH_TO_IMAGE_OUT, save_format='png',subset="training")
    ...
    print(classification_report(y_test.argmax(axis=1), id_pred,
                                target_names=["Merokok", "Tidak_Merokok"]))

    plot_train_history(H)
    # serialize the model to disk
    model.save(os.path.join(MODELS_PATH, "cig_mobilenet_test.h5"), save_format="h5")
