...
model = keras.models.load_model('models/cig_mobilenet.h5')
face_detector = load_cascade_detector()


def video_cig_detector():
    ...
    while True:
        ...
        frame = detect_cig_in_frame(frame)
        # Display the resulting frame
        # show the output frame
        cv2.imshow("Cig detector", frame)
...
def detect_cig_in_frame(frame):
    ...
    if faces_dict["faces_list"]:
        ...
        for i, pred in enumerate(preds):
            cig_or_not, confidence = decode_prediction(pred)
            write_bb(cig_or_not, confidence, faces_dict["faces_rect"][i], frame)
